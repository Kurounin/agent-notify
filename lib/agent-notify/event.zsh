agent_notify_process_normalized() {
  ...
  local source kind session_id session_dir request_id state_key lock_path
  ...
  agent_notify_with_mkdir_lock "$lock_path" agent_notify_transition "$state_key" "$source" "$kind" "$session_dir" "$request_id"
}
